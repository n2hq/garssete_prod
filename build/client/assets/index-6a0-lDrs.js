import{j as e}from"./jsx-runtime-0DLF9kdB.js";import{z as n,u as S,s as k}from"./index-CORT_df8.js";import{r as x,a as R,L as f,c as C}from"./components-DN9bEK-g.js";import{w as E,W as L}from"./WhiteLogo-B5e1sxdi.js";import{u as $}from"./AuthContext-CSAbPGXW.js";import{u as A}from"./NotificationContext-BCSBdBkz.js";import{h as W}from"./lib-BlD1bBbt.js";const b=new RegExp(/^(?=.*?[A-Z])(?=.*?[a-z])(?=.*?[0-9])(?=.*?[#?!()£@$%^&*-]).{8,}$/),D=n.object({password:n.string().min(1,{message:"Please enter new password."}).min(8,{message:"Password must be up to 8 characters."}).regex(b,{message:"Please enter a valid password"}),password2:n.string().min(1,{message:"Please enter new password."}).min(8,{message:"Password must be at least 8 characters."}).regex(b,{message:"Please enter a valid password"})}).superRefine((t,o)=>{t.password!==t.password2&&o.addIssue({code:n.ZodIssueCode.custom,path:["password2"],message:"Your new password don't match"})}),F=({guid:t})=>{var h,g;const[o,j]=x.useState(null);if(!$())return null;const[V,r]=x.useState(!1),c=A();R();const[m,z]=x.useState(!1),v="Please check email provided to continue.",u=l=>{let i=l.target.value,d=l.target.name;j(p=>({...p,[d]:i}))},N=async l=>{r(!0),c.notify("","Working..."),await new Promise(a=>setTimeout(a,1e3)),l.password;const i="https://testgassete.gasimg.com",d=`/api/user/reset_password/${t}`,p=i+d;try{const a=await fetch(p,{method:"PUT",headers:W,body:JSON.stringify(l)});if(a.ok)c.alertCancel("Success!","Password Successfully Changed! Use new password on next login");else{let G=a.json().then(P=>{c.alertCancel("Error!",P.message)})}}catch{return}finally{r(!1)}await new Promise(a=>setTimeout(a,1e3)),r(!1)},{register:w,handleSubmit:y,getValues:B,watch:I,setError:Z,formState:{errors:s,isSubmitting:_}}=S({defaultValues:{},resolver:k(D)});return e.jsxs("div",{className:`w-[80%] h-fit grid grid-cols-1 lg:grid-cols-2
        ...
        bg-cover bg-center min-h-screen flex place-content-center`,children:e.jsx(F,{guid:t})}),Q=()=>{const o=C().guid;return e.jsx("div",{children:e.jsx(T,{userGuid:o})})};export{Q as default};
