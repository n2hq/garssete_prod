import{j as e}from"./jsx-runtime-0DLF9kdB.js";import{u as P,s as k}from"./zod-g2XesUsp.js";import{r,u as E,L as j}from"./components-qdL6XBti.js";import{w as C,W as L}from"./WhiteLogo-BdP7-UZf.js";import{u as _}from"./NotificationContext-jHUYMFh3.js";import{z as F,m as A}from"./lib-q9e4eigB.js";import{z as a}from"./index-C4bLmoCR.js";const $=/^(?=.*[A-Z])(?=.*[a-z])(?=.*\d)(?=.*[\W_]).{8,}$/,W=a.object({email:a.string({message:"Please enter an email"}).min(7,{message:"Email must be greater than 7 characters"}).email({message:"Please enter a valid email"}),password:a.string({message:"Please enter a password"}).min(8,"Password must be at least 8 characters").regex($,"Please enter a valid password"),first_name:a.string({message:"Please enter your first name"}).min(1,{message:"First name must be at least 1 character"}).max(50,{message:"First name must be at most 50 characters"}),lastname:a.string({message:"Please enter your last name"}).min(1,{message:"Last name must be at least 1 character"}).max(50,{message:"Last name must be at most 50 characters"})}),z=()=>{var p,u,g,h;const[R,v]=r.useState(null),[T,o]=r.useState(!1),c=_();E();const[x,N]=r.useState(!1),d="Signup is successful! Please check email provided to complete signup.",l=i=>{let f=i.target.value,b=i.target.name;v(m=>({...m,[b]:f}))},y=async i=>{o(!0),c.notify("",""),await new Promise(s=>setTimeout(s,1e3));const m="https://edition.garssete.com"+"/api/user";try{const s=await fetch(m,{method:"POST",headers:A,body:JSON.stringify(i)});if(s.ok)c.alertCancel("",d),N(!0);else{var w=await s.json();throw new Error(`Error Code: ${s.status} - ${w.message||w.error}`)}}catch(s){c.alertCancel("",s.message);return}finally{o(!1)}},{register:n,handleSubmit:S,getValues:M,watch:U,setError:V,formState:{errors:t,isSubmitting:G}}=P({defaultValues:{},resolver:k(W)});return e.jsxs("div",{className:`w-[80%] h-fit grid grid-cols-1 lg:grid-cols-2
        ...
        bg-cover bg-center min-h-screen flex place-content-center`,children:e.jsx(z,{})}),K=()=>e.jsx("div",{children:e.jsx(B,{})});export{K as default};
