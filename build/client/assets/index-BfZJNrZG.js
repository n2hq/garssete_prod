import{j as e}from"./jsx-runtime-0DLF9kdB.js";import{u as P,s as k}from"./zod-g2XesUsp.js";import{r,u as E,L as j}from"./components-qdL6XBti.js";import{w as C,W as L}from"./WhiteLogo-B8d5qcGj.js";import{u as _}from"./NotificationContext-q7hemzCX.js";import{w as F,n as A}from"./lib-W9JEsCxK.js";import{z as a}from"./index-C4bLmoCR.js";const $=/^(?=.*[A-Z])(?=.*[a-z])(?=.*\d)(?=.*[\W_]).{8,}$/,W=a.object({email:a.string({message:"Please enter an email"}).min(7,{message:"Email must be greater than 7 characters"}).email({message:"Please enter a valid email"}),password:a.string({message:"Please enter a password"}).min(8,"Password must be at least 8 characters").regex($,"Please enter a valid password"),first_name:a.string({message:"Please enter your first name"}).min(1,{message:"First name must be at least 1 character"}).max(50,{message:"First name must be at most 50 characters"}),lastname:a.string({message:"Please enter your last name"}).min(1,{message:"Last name must be at least 1 character"}).max(50,{message:"Last name must be at most 50 characters"})}),B=()=>{var p,u,g,h;const[T,v]=r.useState(null),[z,o]=r.useState(!1),c=_();E();const[x,N]=r.useState(!1),d="Signup is successful! Please check email provided to complete signup.",l=i=>{let f=i.target.value,w=i.target.name;v(m=>({...m,[w]:f}))},y=async i=>{o(!0),c.notify("",""),await new Promise(s=>setTimeout(s,1e3));const m="https://edition.garssete.com"+"/api/user";try{const s=await fetch(m,{method:"POST",headers:A,body:JSON.stringify(i)});if(s.ok)c.alertCancel("",d),N(!0);else{var b=await s.json();throw new Error(`Error Code: ${s.status} - ${b.message||b.error}`)}}catch(s){c.alertCancel("",s.message);return}finally{o(!1)}},{register:n,handleSubmit:S,getValues:M,watch:U,setError:V,formState:{errors:t,isSubmitting:G}}=P({defaultValues:{},resolver:k(W)});return e.jsxs("div",{className:`w-[80%] h-fit grid grid-cols-1 lg:grid-cols-2
        ...
        bg-cover bg-center min-h-screen flex place-content-center`,children:e.jsx(B,{})}),K=()=>e.jsx("div",{children:e.jsx(R,{})});export{K as default};
