import{j as e}from"./jsx-runtime-0DLF9kdB.js";import{z as a,u as k,s as P}from"./index-CORT_df8.js";import{r,a as E,L as j}from"./components-DN9bEK-g.js";import{w as C,W as L}from"./WhiteLogo-CE_Ma9WS.js";import{u as _}from"./NotificationContext-BES0JERW.js";import{f as F,h as A}from"./lib-CdsGGIGA.js";const $=/^(?=.*[A-Z])(?=.*[a-z])(?=.*\d)(?=.*[\W_]).{8,}$/,W=a.object({email:a.string({message:"Please enter an email"}).min(7,{message:"Email must be greater than 7 characters"}).email({message:"Please enter a valid email"}),password:a.string({message:"Please enter a password"}).min(8,"Password must be at least 8 characters").regex($,"Please enter a valid password"),first_name:a.string({message:"Please enter your first name"}).min(1,{message:"First name must be at least 1 character"}).max(50,{message:"First name must be at most 50 characters"}),lastname:a.string({message:"Please enter your last name"}).min(1,{message:"Last name must be at least 1 character"}).max(50,{message:"Last name must be at most 50 characters"})}),B=()=>{var p,u,g,h;const[T,v]=r.useState(null),[z,o]=r.useState(!1),i=_();E();const[x,N]=r.useState(!1),d="Signup is successful! Please check email provided to complete signup.",l=c=>{let f=c.target.value,w=c.target.name;v(m=>({...m,[w]:f}))},y=async c=>{o(!0),i.notify("",""),await new Promise(s=>setTimeout(s,1e3));const m="https://tynk.cc"+"/api/user";try{const s=await fetch(m,{method:"POST",headers:A,body:JSON.stringify(c)});if(s.ok)i.alertCancel("",d),N(!0);else{var b=await s.json();throw new Error(`Error Code: ${s.status} - ${b.message||b.error}`)}}catch(s){i.alertCancel("",s.message);return}finally{o(!1)}},{register:n,handleSubmit:S,getValues:M,watch:U,setError:V,formState:{errors:t,isSubmitting:G}}=k({defaultValues:{},resolver:P(W)});return e.jsxs("div",{className:`w-[80%] h-fit grid grid-cols-1 lg:grid-cols-2
